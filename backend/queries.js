...
const undoSwipe = (req, res) => {
  const idUser = req.body.data.idUser;
  const idMovie = req.body.data.idMovie;
  ...
  pool.query(
    ...
    SET movie_selected = 
      CASE 
        WHEN $2 = ANY(movie_selected) THEN array_remove(movie_selected, $2) 
        ELSE movie_selected 
      END 
    WHERE id = $1 
    RETURNING *;
...
const removeMatch = (req, res) => {
  const idApp = req.body.data.idApp;
  const idMovie = req.body.data.idMovie;
  ...
  pool.query(
    ...
    SET match = 
      CASE 
        WHEN $2 = ANY(match) THEN array_remove(match, $2) 
        ELSE match 
      END 
    WHERE id = $1 
    RETURNING *;
    ...
    (err, result) => {
      if (err) {
        ...
        res.status(500).json({ error: "Error updating lobby" });
        return;
      }
      res.status(200).json(result.rows[0]);
    }
    ...
    pool.query(
      ...
      (err, result) => {
        ...
        if (result.rows.length === 0) {
          return res
            .status(404)
            .json({ error: "No lobby found for this application ID" });
        }

        let allMovieSelections = [];
        if (result.rows.length > 0) {
          for (let i = 0; i < result.rows.length; i++) {
            allMovieSelections = allMovieSelections.concat(
              result.rows[i].movie_selected
            );
          }
        }

        const matchingMovies = allMovieSelections.filter((id) => id == movie);
        const matchingMoviesBoolean = matchingMovies.length === result.rows.length && result.rows.length > 1;
        ...
        `,
            ...
            async (err, result) => {
              if (err) {
                ...
                return res
                  .status(500)
                  .json({ error: "Error updating lobby match" });
              }

              // Emit the match event to all users in the lobby
              io.to(`lobby_${idApp}`).emit('match_update', { match: await movieById(movie) });
